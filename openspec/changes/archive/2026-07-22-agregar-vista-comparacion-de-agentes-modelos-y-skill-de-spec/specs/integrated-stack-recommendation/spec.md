...
La vista integrada SHALL permitir seleccionar el contexto canónico del agente, la
fase o carga del SDLC y el escenario operativo de la capa de
...
`alternative`, `pilot` o `none`, y el resolver MUST NOT inferir esos roles desde
el orden o texto visible de una tabla.
...
Cada combinación SHALL validar agente-modelo y agente-framework mediante matrices
canónicas antes de presentarse como recomendación final.
...
Cuando una combinación quede bloqueada, el sistema SHALL enumerar candidatos con
soporte nativo o condicionado documentado para el componente conflictivo y MUST
NOT calificarlos como mejores si ninguna fuente establece ese orden.
...
foco visible y anunciar cambios sustantivos sin depender exclusivamente de color.
